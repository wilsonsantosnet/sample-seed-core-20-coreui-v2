...
@Component({
    selector: 'app-sampleproduct',
    templateUrl: './sampleproduct.component.html',
    styleUrls: ['./sampleproduct.component.css'],
})
export class SampleProductComponent extends ComponentBase implements OnInit, OnDestroy {
    ...
    constructor(private sampleProductService: SampleProductService, private router: Router, private ref: ChangeDetectorRef) {
    ...
    ngOnInit() {
        ...
        this.sampleProductService.OnHide(this.saveModal, this.editModal, () => { this.hideComponents() });

        this.sampleProductService.get(this.vm.modelFilter).subscribe((result) => {
        ...
        if (this._navigatioModal)
            LocationHistoryService.saveLocal("sampleproduct");
    ...
    updateCulture(culture: string = null)
    {
        ...
        this.sampleProductService.updateCultureMain(culture).then((infos: any) => {
    ...
    public onFilter(modelFilter: any) {
        ...
        this.sampleProductService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
    ...
    public onExport() {
        this.sampleProductService.export().subscribe((result) => {
    ...
    public onCreate() {
        ...
        this.navigateStrategy(this.saveModal, this.router, "/sampleproduct/create");
    }
    ...
    public onEdit(model: any) {
        ...
        if (model)  {
            newModel = { sampleId: model.sampleId, productId: model.productId }
        }

        if (!this._navigatioModal) {
            this.navigateStrategy(this.editModal, this.router, "/sampleproduct/edit/" + newModel.id);
        }
        else {
            this.sampleProductService.get(newModel).subscribe((result) => {
    ...
    public onSave(model: any) {
        ...
        this.sampleProductService.save(model).subscribe((result) => {
            ...
            this.vm.filterResult = this.vm.filterResult.filter(function (model) {
                return  model.sampleId != result.data.sampleId ||  model.productId != result.data.productId;
            });
    ...
    public onDetails(model: any) {
        ...
        if (!this._navigatioModal) {
            this.navigateStrategy(this.editModal, this.router, "/sampleproduct/details/" + newModel.id);
        }
        else {
            this.sampleProductService.get(newModel).subscribe((result) => {
    ...
    public onPrint(model: any) {
        this.router.navigate(['/sampleproduct/print', model.sampleId]);
    }
    ...
    public onDeleteConfimation(model: any) {
        ...
        var conf = GlobalService.operationExecutedParameters(
            ...
            () => {
                ...
                this.sampleProductService.delete(newModel).subscribe((result) => {
                    this.vm.filterResult = this.vm.filterResult.filter(function (model) {
                        return  model.sampleId != result.data.sampleId ||  model.productId != result.data.productId;
                    });
                    this.vm.summary.total = this.vm.filterResult.length
    ...
    public onPageChanged(pageConfig: any) {
        ...
        let modelFilter = this.sampleProductService.pagingConfig(this.vm.modelFilter, pageConfig);
        this.sampleProductService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
        });
    ...
    public onOrderBy(order: any) {
        ...
        let modelFilter = this.sampleProductService.orderByConfig(this.vm.modelFilter, order);
        this.sampleProductService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
        });
