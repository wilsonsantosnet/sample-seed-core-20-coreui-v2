...
{
	[Authorize]
    [Route("api/samplestandart/more")]
    public class SampleStandartMoreController : Controller
    {

        private readonly ISampleStandartRepository _rep;
        ...
        public async Task<IActionResult> Get([FromQuery]SampleStandartFilter filters)
        {
            ...
            try
            {
				...
				if (filters.FilterBehavior == FilterBehavior.Export)
                {
                    ...
                    var file = export.ExportFile(this.Response, searchResult, "SampleStandart", this._env.RootPath);
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody]IEnumerable<SampleStandartDtoSpecialized> dtos)
        {
            ...
            {
                return result.ReturnCustomException(ex,"Seed - SampleStandart", dtos, new ErrorMapCustom());
            }

        }

		[HttpPut]
        public async Task<IActionResult> Put([FromBody]IEnumerable<SampleStandartDtoSpecialized> dtos)
        {
            var result = new HttpResult<SampleStandartDto>(this._logger);
            try
            {
